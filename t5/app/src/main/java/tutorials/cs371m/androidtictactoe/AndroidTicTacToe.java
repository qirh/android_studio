...
public class AndroidTicTacToe extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.about:
                ...
                return true;
            //reset will repaint the screen as well, will not reset the diff
            case R.id.reset:
                startNewGame();
                resetScores();
                resetBoard();
    ...
    protected void onResume() {
        ...
        if (!mHumansTurnToMove && !mGameOver) {
            startComputerDelay();
        }
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        for (WinData.Outcome outcome : WinData.Outcome.values()) {
            outState.putInt(outcome.name(), mWinData.getCount(outcome));
        }
    ...
    @Override public void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < counters.length; i++) {
            counters[i] = savedInstanceState.getInt(outcomes[i].name());
        }
    ...
    */
    ...
    @Override protected void onStop() {
        ...
        editor.putString("diff", mGame.getDifficultyLevel().toString());

        char[] c = mGame.getBoardState();
        for (int i = 0; i< c.length; i++) {
            editor.putString("Board"+i, c[i] + "");
        }
    ...
    private void restoreDifficulty() {
        ...
        diff = TicTacToeGame.DifficultyLevel.valueOf(sharedPref.getString("diff", "Expert"));
        mGame.setDifficultyLevel(diff);
    }
    private void restoreBoard() {
        SharedPreferences sharedPref = getPreferences(MODE_PRIVATE);

        for (int i = 0; i < mGame.getBoardState().length; i++) {
            mGame.setMove(sharedPref.getString("Board"+i, " ").charAt(0), i);
        }
    }
